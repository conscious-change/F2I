...
process_image() {
  ...
  for size in "${SIZES[@]}"; do
    ...
    convert "$img" -resize ${size}x -quality $QUALITY "${OUTPUT_DIR}/${name}-${size}.${extension}"
...
echo ""
echo "Next steps:"
echo "1. Edit _includes/responsive-image.html to uncomment the advanced picture element code"
echo "2. Edit _includes/responsive-background.html to uncomment the responsive background code"
echo "3. Test the website on different devices and browsers"
